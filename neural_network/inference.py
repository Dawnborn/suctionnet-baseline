...
def get_suction_from_heatmap(depth_img, heatmap, camera_info):
    ...
    points_sampled = np.concatenate([suction_points, points_sampled], axis=0) # 加回suction_points
    pc_voxel_sampled.points = o3d.utility.Vector3dVector(points_sampled)
    pc_voxel_sampled.estimate_normals(o3d.geometry.KDTreeSearchParamRadius(0.015), fast_normal_computation=False) # 提取点云法线
    pc_voxel_sampled.orient_normals_to_align_with_direction(np.array([0., 0., -1.])) # 法线统一与相机坐标系z反向（即法线朝向相机）
    pc_voxel_sampled.normalize_normals() # 正则化法线
...
def inference_one_view(rgb_file, depth_file, meta_file, scene_idx, anno_idx):
    ...
    heatmap = F.conv2d(heatmap, kernel, padding=(kernel.shape[2] // 2, kernel.shape[3] // 2)).squeeze().numpy() # 1 1 720 1280
